...
client = InferenceHTTPClient(
    ...
    api_key="atL3rkIRqnsfe1BXXGHf",  # consider moving to env var
)
...
PLAYER_CLASS_NAMES = {"players"}
BALL_CLASS_NAMES = {"tennis-ball"}

MIN_PLAYER_CONF = 0.6
MIN_BALL_CONF = 0.01

ASSIGN_THRESH = 80.0  # pixels for nearest-neighbor association
# ------------------------
...
def parse_workflow_preds(result):
    ...
    preds = pred_root.get("predictions", [])
...
def run(video_path: str, output_path: str, task: str) -> dict:
    ...
    Currently supports:
      - task == "track_players": simple nearest-neighbor tracking for players,
        draws tracks and IDs; returns number of tracks and frames processed.
    """
    if task != "track_players":
        raise ValueError(f"Unsupported tennis task: {task}")
    ...
    while True:
        ...
        if frame_idx % stride == 0:
            ...
            print(
                ...
                f"{len(players)} players, {len(balls)} balls"
            )
        ...
        for tid, pts in player_tracks.items():
            ...
            if len(pts_list) > 1:
                ...
                cv2.circle(
                    boxed, (int(x_last), int(y_last)), 4, (255, 0, 0), -1
                )
                cv2.putText(
                    boxed,
                    f"P{tid}",
